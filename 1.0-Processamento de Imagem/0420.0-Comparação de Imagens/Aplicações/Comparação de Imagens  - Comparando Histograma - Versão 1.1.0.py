...
#
def Similaridade ( imgTmpOriginal, imgTmpComparada, Altura, Largura):
    imgHSVOriginal = cv.cvtColor(imgTmpOriginal, cv.COLOR_BGR2HSV)
    imgHSVComparada = cv.cvtColor(imgTmpComparada, cv.COLOR_BGR2HSV)

    histBase = cv.calcHist([imgHSVOriginal], [0,1], None, [180,256], [0,180,0,256])
    cv.normalize(histBase, histBase, alpha=0, beta=1, norm_type=cv.NORM_MINMAX)
    histComparada = cv.calcHist([imgHSVComparada], [0,1], None, [180,256], [0,180,0,256])
    cv.normalize(histComparada, histComparada, alpha=0, beta=1, norm_type=cv.NORM_MINMAX)

    Similaridade = cv.compareHist(histBase, histComparada, cv.HISTCMP_CHISQR)
    return (Similaridade) 
...
########################################################################
#
DiferencaOriginalImagem1 = Similaridade ( imgViolaoOriginal, imgViolaoOpcao1, 400, 160)
DiferencaOriginalImagem2 = Similaridade ( imgViolaoOriginal, imgViolaoOpcao2, 400, 160)
DiferencaOriginalImagem3 = Similaridade ( imgViolaoOriginal, imgViolaoOpcao3, 400, 160)
DiferencaOriginalImagem4 = Similaridade ( imgViolaoOriginal, imgViolaoOpcao4, 400, 160)
DiferencaOriginalImagem5 = Similaridade ( imgViolaoOriginal, imgViolaoOpcao5, 400, 160)
